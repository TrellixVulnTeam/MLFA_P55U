    ...
    for control in controls:
        ...
        for name, group in groups:
            ...
            for i in range(num_trials):
                ...
                if param == 'mode':
                    diff = np.mean(param_function(bB)) - \
                        np.mean(param_function(bA))
                ...
                if alternate == 'lower':
                    ...
                    print(
                        "Error: invalid alternative hypothesis. Options are 'both', 'higher', or 'lower'")
                    break
...
def check_normality(data, cols, display_results=True, drop_na=True):
    ...
    for col in cols:
        ...
        info[col]['K-S']['Statistic'], info[col]['K-S']['p-value'] = stats.kstest(
            x, 'norm')
        info[col]['Shapiro-Wilk']['Statistic'], info[col]['Shapiro-Wilk']['p-value'] = stats.shapiro(
            x)
    ...
    if display_results == True:
        ...
        display_side_by_side(test_results, ad_results,
                             jb_results.T, names=names)
...
def compare_groups(dataframe, feature, targets, control_group=None, alpha=0.05, p_adjust=False, show_groups=True, **kwargs):
    ...
    for k, v in kwargs.items():
        if k not in ['figsize', 'edgecolor']:
            raise TypeError(
                "compare_groups got an unexpected keyword argument {}".format(k))
        else:
            if k == 'figsize':
                figsize = v
            elif k == 'edgecolor':
    ...
    for target in targets:
            ...
            if group[0] == control_group:
                ...
                if size == 1:
                    ...
                    stat, p = stats.ttest_ind(
                        test_group, control, equal_var=False)
        ...
        print('Testing {} groups for statistically significant effects on {}'.format(
            feature, target))
        ...
        ax1.axhline(y=alpha, ls='-.',
                    label='alpha: {}'.format(alpha), alpha=0.7)
        ax1.set(xlabel='')
        ax1.legend()
        if len(grouped) - 1 == 1:
            ax2.scatter(X,
            ...
            ax2.plot(X, info.loc['effect size'],
                     color='g', label='effect size')
...
def display_side_by_side(*args, names=None):
    ...
    display_html(html_str.replace(
        'table', 'table style="display:inline" cellpadding=100'), raw=True)
...
def do_a_linreg(dataframe, features, target):
    ...
    if linreg_type == 'simple':
        linreg = LinearRegression().fit(np.reshape(
            [dataframe[feature]], (-1, 1)), dataframe[target])
        X = np.linspace(dataframe[feature].min(),
                        dataframe[feature].max(), 500).reshape(-1, 1)
        y = linreg.predict(X)
        ...
        plt.title("Linear Regression for {} ~ {}".format(
            target, feature), color=text_color)
    ...
    sm.graphics.qqplot(model.resid, dist=stats.norm,
                       line='45', fit=True, ax=ax1)
...
def ecdf(data, group_by=None, targets=None, ax=None, **kwargs):
    ...
    for k, v in kwargs.items():
        if k not in ['linewidth']:
            raise TypeError(
                'ecdf got an unexpeted keyword argument: {}'.format(k))
        else:
            if k == 'linewidth':
                linewidth = v
    ...
    if type(data) == pd.core.groupby.generic.DataFrameGroupBy:
        for variable in variables:
            ...
            ax.legend()
            plt.title("ECDF for feature: {}".format(
                variable), color=text_color)
            plt.show()
...
def explore_groups(dataframe, feature, target):
    percentages = {}
    group_stats, groups = group_hist(
        dataframe, feature, target, show_hist=False, return_groups=True)
    for name, group in groups:
    ...
    ax2.plot(list(range(len(group_stats))), group_stats.Size, label='Order Frequency',
             c='green', alpha=0.7)
    ax2.legend()
    ax3 = group_stats.Mean.plot(
        kind='bar', label='Avg {}'.format(target), alpha=0.6, color='blue')
    ax3.legend()
    plt.show()
...
def goldfeld_quandt(dataframe, target, model, ax=None, alternative='two-sided'):
    ...
    middle_10percent_indices = dataframe[(dataframe[target] >= lwr_thresh) & (
        dataframe[target] <= upr_thresh)].index
    ...
    ax.set_title("Residuals versus {} predictions".format(
    ...
    #p = f_test(var1, var2, df1, df2)
    results = pd.DataFrame(
        index=['Goldfeld-Quandt'], columns=['F_statistic', 'p_value'])
...
def group_hist(data, feature, target, show_hist=True, return_groups=False):
    ...
    if show_hist:
        ...
        grouped.hist(density=True, histtype='step',
                     alpha=1, stacked=True, lw=2)
...
def make_boxplot(dataframe, x, y):
    ...
    plt.title("Side by side comparison of all group distributions:",
              color=text_color)
...
def metropolis(data1, theta_seed1, theta_std1, data2=None, theta_seed2=None, theta_std2=None, samples=10000):
    ...
    for i in range(samples):
        ...
        for datum in data1:
            ...
            if i == 0:
                pd_curr = stats.norm.pdf(
                    x=datum, loc=theta_curr1, scale=scaleA)
                likelihood_curr1 *= pd_curr

        posterior_prop1 = likelihood_prop1 * \
            stats.norm.pdf(x=theta_prop1, loc=theta_curr1, scale=theta_std1)
        if i == 0:
            posterior_curr1 = likelihood_curr1 * \
                stats.norm.pdf(x=theta_curr1, loc=theta_curr1,
                               scale=theta_std1)
        ...
        if data2 is not None:
            for datum in data2:
                pd_prop = stats.norm.pdf(
                    x=datum, loc=theta_prop2, scale=scaleB)
                likelihood_prop2 *= pd_prop
                if i == 0:
                    pd_curr = stats.norm.pdf(
                        x=datum, loc=theta_curr2, scale=scaleB)
                    likelihood_curr2 *= pd_curr

            posterior_prop2 = likelihood_prop2 * \
                stats.norm.pdf(x=theta_prop2, loc=theta_curr2,
                               scale=theta_std2)
            if i == 0:
                posterior_curr2 = likelihood_curr2 * \
                    stats.norm.pdf(
                        x=theta_curr2, loc=theta_curr2, scale=theta_std2)
    ...
    if data2 is not None:
        ...
        ax2.axvline(x=np.mean(posterior_thetas1), ls=':',
        ...
        ax4.axvline(x=np.mean(posterior_thetas2), ls=':',
        ...
        conf_interval = stats.norm.interval(
            0.95, loc=sizes_mu, scale=sizes_std)
        ...
        ax1.hist(data1, density=True, alpha=0.6)
        ax1.axvline(x=calc_mean1, ls=':', color='g',
                    label='Measured mean: {}'.format(calc_mean1))
        ...
        ax2.axvline(x=mcmc_theta1, ls=':', color='g',
                    label='MCMC mean: {}'.format(mcmc_theta1))
        ax2.legend()
        plt.show()
    ...
    for name, group in dataframe.groupby(feature)[target]:
        ...
        if name == control:
            ...
            if N > 50:
                ...
                control_group = np.random.choice(
                    control_group, size=n, replace=False)
        else:
            ...
            for grouping in groupings:
                ...
                if alternate == 'lower':
                    ...
                    print(
                        "Error: invalid alternate hypothesis. Options are 'both', 'lower', 'higher'")
...
def winsorize_df(df, col, percent):
    ...
    print("Length of old DataFrame:", len(df))
    dataframe = df.copy()
    dataframe[col] = stats.mstats.winsorize(
        df[col], limits=(percent, percent), inplace=False)
